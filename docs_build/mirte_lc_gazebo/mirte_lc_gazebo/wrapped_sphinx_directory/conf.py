...
if os.path.isfile('/workspaces/lcr/src/mirte_lc/docs_build/mirte_lc_gazebo/mirte_lc_gazebo/wrapped_sphinx_directory/__conf.py'):
    try:
        exec(open("/workspaces/lcr/src/mirte_lc/docs_build/mirte_lc_gazebo/mirte_lc_gazebo/wrapped_sphinx_directory/__conf.py").read())
        confpy_succeeded = True
        print('[rosdoc2] Using user supplied conf.py')
    except Exception as e:
        print(f'[rosdoc2] *** Warning *** conf.py for package mirte_lc_gazebo generated error: '
              + str(e) + '. Falling back to default generated conf.py.')
if not confpy_succeeded:
    exec(open("/workspaces/lcr/src/mirte_lc/docs_build/mirte_lc_gazebo/mirte_lc_gazebo/wrapped_sphinx_directory/__conf_default.py").read())

## Copy any templates to the wrapped location.
for t_dir in templates_path:
    source_t_dir = os.path.join('/workspaces/lcr/src/mirte_lc/docs_build/mirte_lc_gazebo/mirte_lc_gazebo/wrapped_sphinx_directory', t_dir)
    target_t_dir = os.path.join('/workspaces/lcr/src/mirte_lc/docs_build/mirte_lc_gazebo/mirte_lc_gazebo/wrapped_sphinx_directory', t_dir)
...
if rosdoc2_settings.get('enable_exhale', is_doxygen_invoked):
    ...
    exhale_args.update({
        ...
        "containmentFolder": "/workspaces/lcr/src/mirte_lc/docs_build/mirte_lc_gazebo/mirte_lc_gazebo/wrapped_sphinx_directory/generated",
