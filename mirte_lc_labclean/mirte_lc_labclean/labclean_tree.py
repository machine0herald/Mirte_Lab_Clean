...
def create_root() -> py_trees.behaviour.Behaviour:
    ...
    # 1.4: Detected bounding boxes topic to Blackboard #
    boundingbox2bb = py_trees_ros.subscribers.ToBlackboard(
        name="Detected2BB",
        topic_name="/detected_objects/detected",
        qos_profile=py_trees_ros.utilities.qos_profile_unlatched(),
        variable_name="objects_detected",
    )
    ...
    # 2.2: Scan Task #
    labclean = py_trees.composites.Sequence(name="Scan", memory=True)

    # 2.2.1: Is Scan Requested? #
    is_explore_done = py_trees.behaviours.CheckBlackboardVariableValue(
        name="???",
        check=py_trees.common.ComparisonExpression(
            variable="???", value=True, operator=operator.eq
        ),
    )
    ...
    scan_or_be_cancelled = py_trees.composites.Selector(
        name="Scan or Be Cancelled", memory=False
    )
    ...
    is_cancel_requested = py_trees.behaviours.CheckBlackboardVariableValue(
        name="Cancel?",
        check=py_trees.common.ComparisonExpression(
            variable="event_cancel_button", value=True, operator=operator.eq
        ),
    )
    ...
    move_out_and_scan = py_trees.composites.Sequence(
        name="Move Out and Scan", memory=True
    )
    ...
    scanning = py_trees.composites.Parallel(
        name="Scanning", policy=py_trees.common.ParallelPolicy.SuccessOnOne()
    )
    ...
    class SendResult(py_trees.behaviour.Behaviour):
        ...
        def __init__(self, name: str):
            ...
            self.blackboard.register_key(
                key="scan_result", access=py_trees.common.Access.READ
            )
