...
if '/workspaces/lcr/src/mirte_lc/mirte_lc/mirte_lc' != 'None':
    sys.path.insert(0, os.path.abspath(os.path.join('/workspaces/lcr/src/mirte_lc/mirte_lc/mirte_lc', '..')))
...
copyright = 'The <mirte_lc> Contributors. License: TODO: License declaration'
author = """root"""
...
breathe_projects = {}
master_doc = 'index'
intersphinx_mapping = { 'mirte_lc_moveit_cpp': ('http://docs.ros.org/en/latest/p/mirte_lc_moveit_cpp/', '/workspaces/lcr/src/mirte_lc/cross_reference/mirte_lc_moveit_cpp/objects.inv'),
...
if os.path.isfile('/workspaces/lcr/src/mirte_lc/docs_build/mirte_lc/mirte_lc/wrapped_sphinx_directory/__conf.py'):
    try:
        exec(open("/workspaces/lcr/src/mirte_lc/docs_build/mirte_lc/mirte_lc/wrapped_sphinx_directory/__conf.py").read())
        confpy_succeeded = True
        print('[rosdoc2] Using user supplied conf.py')
    except Exception as e:
        print(f'[rosdoc2] *** Warning *** conf.py for package mirte_lc generated error: '
              + str(e) + '. Falling back to default generated conf.py.')
if not confpy_succeeded:
    exec(open("/workspaces/lcr/src/mirte_lc/docs_build/mirte_lc/mirte_lc/wrapped_sphinx_directory/__conf_default.py").read())

## Copy any templates to the wrapped location.
for t_dir in templates_path:
    source_t_dir = os.path.join('/workspaces/lcr/src/mirte_lc/docs_build/mirte_lc/mirte_lc/wrapped_sphinx_directory', t_dir)
    target_t_dir = os.path.join('/workspaces/lcr/src/mirte_lc/docs_build/mirte_lc/mirte_lc/wrapped_sphinx_directory', t_dir)
...
if rosdoc2_settings.get('enable_autodoc', True):
    ...
    import importlib
    for exec_depend in ['moveit', 'ros2_control', 'foxglove_bridge', 'ros2_controllers', 'moveit_servo', 'moveit_visual_tools', 'gazebo_ros_pkgs', 'slam_toolbox', 'cv_bridge', 'octomap_server', 'topic_tools', 'joint_state_publisher', 'joint_state_publisher_gui', 'navigation2', 'rosbridge_server', 'tf_transformations', 'usb_cam', 'web_video_server', 'rosbag2_storage_mcap', 'py_trees', 'py_trees_ros_interfaces', 'py_trees_ros', 'py_trees_ros_tutorials', 'py_trees_ros_viewer', 'pcl_ros']:
        try:
...
if rosdoc2_settings.get('enable_exhale', is_doxygen_invoked):
    ...
    exhale_args.update({
        ...
        "containmentFolder": "/workspaces/lcr/src/mirte_lc/docs_build/mirte_lc/mirte_lc/wrapped_sphinx_directory/generated",
