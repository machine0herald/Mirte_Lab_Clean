...
if '/workspaces/lcr/src/mirte_lc/mirte_lc_labclean/mirte_lc_labclean' != 'None':
    sys.path.insert(0, os.path.abspath(os.path.join('/workspaces/lcr/src/mirte_lc/mirte_lc_labclean/mirte_lc_labclean', '..')))

# -- Project information -----------------------------------------------------

project = 'mirte_lc_labclean'
ros_distro = os.environ.get('ROS_DISTRO')
if ros_distro:
    project += ': ' + ros_distro.capitalize()

# The docs say year and author but we have this and it seems more relevant.
copyright = 'The <mirte_lc_labclean> Contributors. License: TODO: License declaration'
author = """root"""
...
if os.path.isfile('/workspaces/lcr/src/mirte_lc/docs_build/mirte_lc_labclean/mirte_lc_labclean/wrapped_sphinx_directory/__conf.py'):
    try:
        exec(open("/workspaces/lcr/src/mirte_lc/docs_build/mirte_lc_labclean/mirte_lc_labclean/wrapped_sphinx_directory/__conf.py").read())
        confpy_succeeded = True
        print('[rosdoc2] Using user supplied conf.py')
    except Exception as e:
        print(f'[rosdoc2] *** Warning *** conf.py for package mirte_lc_labclean generated error: '
              + str(e) + '. Falling back to default generated conf.py.')
if not confpy_succeeded:
    exec(open("/workspaces/lcr/src/mirte_lc/docs_build/mirte_lc_labclean/mirte_lc_labclean/wrapped_sphinx_directory/__conf_default.py").read())

## Copy any templates to the wrapped location.
for t_dir in templates_path:
    source_t_dir = os.path.join('/workspaces/lcr/src/mirte_lc/docs_build/mirte_lc_labclean/mirte_lc_labclean/wrapped_sphinx_directory', t_dir)
    target_t_dir = os.path.join('/workspaces/lcr/src/mirte_lc/docs_build/mirte_lc_labclean/mirte_lc_labclean/wrapped_sphinx_directory', t_dir)
...
if rosdoc2_settings.get('enable_autodoc', True):
    ...
    import importlib
    for exec_depend in ['rclpy', 'geometry_msgs', 'sensor_msgs', 'slam_toolbox']:
        try:
...
if rosdoc2_settings.get('enable_exhale', is_doxygen_invoked):
    ...
    exhale_args.update({
        ...
        "containmentFolder": "/workspaces/lcr/src/mirte_lc/docs_build/mirte_lc_labclean/mirte_lc_labclean/wrapped_sphinx_directory/generated",
